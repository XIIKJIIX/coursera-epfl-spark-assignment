...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    val questions = postings.filter(p => p.postingType == 1 && p.parentId.isEmpty)
      .map(p => (p.id, p))
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    def firstLangInTag(tag: String): Option[Int] = langs.indexOf(tag) match {
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      val countMap = vs.map(_._1 / langSpread).groupBy(identity).mapValues(_.size)
      val topLangIdx = countMap.maxBy(_._2)._1
      val langLabel: String   = langs(topLangIdx) // most common language in the cluster
      val langPercent: Double = countMap(topLangIdx) * 100 / vs.size // percent of the questions in the most common language
